...
struct LegionsOfDarknessTests {
  ...
  func adapterCreatesAllPieces() {
    ...
    let armyPieces = pieces.filter { $0.label?.hasPrefix("army:") == true }
  ...
  func adapterMapsArmyPositions() {
    ...
    let eastPiece = LoDPieceAdapter.pieces().first { $0.label == "army:east" }!
  ...
  func adapterMapsMorale() {
    ...
    let graph = LoDGraph.board()
    let section = LoDPieceAdapter.section(from: state, graph: graph)
    let moralePiece = LoDPieceAdapter.pieces().first { $0.label == "morale" }!
  ...
  func adapterMapsTimePosition() {
    ...
    let graph = LoDGraph.board()
    let section = LoDPieceAdapter.section(from: state, graph: graph)
    let timePiece = LoDPieceAdapter.pieces().first { $0.label == "time" }!

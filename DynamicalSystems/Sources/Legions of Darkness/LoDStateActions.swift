...
extension LoD.State {
  ...
  // MARK: - Concrete Spell Action Enumeration

  /// All army slots that can be targeted by a given arcane spell.
  func targetableSlotsForArcaneSpell(_ spell: LoD.SpellType) -> [LoD.ArmySlot] {
    LoD.ArmySlot.allCases.filter { slot in
      armyPosition[slot] != nil && canTargetWithArcaneSpell(spell, targetTrack: slot.track)
    }
  }

  /// All army slots that have armies on the board (for divine spells with no track restriction).
  private var occupiedArmySlots: [LoD.ArmySlot] {
    LoD.ArmySlot.allCases.filter { armyPosition[$0] != nil }
  }

  // swiftlint:disable cyclomatic_complexity function_body_length
  /// Enumerate all concrete parameterized actions for casting a spell.
  /// Each returned action has fully-specified params (targets, heroes, defenders, etc.)
  /// so that the MCTS and UI can present one action per valid combination.
  func concreteSpellActions(spell: LoD.SpellType, heroic: Bool) -> [LoD.Action] {
    var result: [LoD.Action] = []

    func makeAction(_ params: LoD.SpellCastParams) -> LoD.Action {
      .magic(.castSpell(spell, heroic: heroic, params))
    }
    ...
    case .cureWounds:
      let wounded = Array(heroWounded).sorted { $0.rawValue < $1.rawValue }
      if heroic {
        for hero in wounded {
          var params = LoD.SpellCastParams()
          params.heroes = [hero]
        ...
        for idx in 0..<wounded.count {
          for jdx in (idx + 1)..<wounded.count {
            var params = LoD.SpellCastParams()
            params.heroes = [wounded[idx], wounded[jdx]]
            result.append(makeAction(params))
          }
        }
      } else {
    ...
    case .massHeal:
      ...
      if heroic {
        for dtype in types {
          var params = LoD.SpellCastParams()
          params.defenders = [dtype]
          result.append(makeAction(params))
        }
        for idx in 0..<types.count {
          for jdx in (idx + 1)..<types.count {
            var params = LoD.SpellCastParams()
            params.defenders = [types[idx], types[jdx]]
            result.append(makeAction(params))
          }
        }
      } else {
    ...
    case .raiseDead:
      let types = LoD.DefenderType.allCases
      let deadHeroes = Array(heroDead).sorted { $0.rawValue < $1.rawValue }
      if heroic {
        // Defenders only (pairs)
        for idx in 0..<types.count {
          for jdx in (idx + 1)..<types.count {
            var params = LoD.SpellCastParams()
            params.defenders = [types[idx], types[jdx]]
    ...
    case .inspire:
      if heroic || canCastInspireNormal() {
        result.append(makeAction(LoD.SpellCastParams()))
      }

    case .chainLightning, .fortune:
      // Sub-pages handle multi-step targeting later; single placeholder action
      result.append(makeAction(LoD.SpellCastParams()))
    }
